...
int h, hass, dehas;        // variable for humiditiy
...
dry = EEPROM.read(207);  
  ...
  lcd.setCursor(2,0);  
  lcd.print("clock, thermostat");
  lcd.setCursor(2,1);  
  lcd.print("& hygrostat v.1.4");
...
if (heat == 0) 
{
 ...
 if (te > tes) lcd.print(" (t>)");
 if (tes - dete > te) lcd.print(" (t<)");
 if ((te <= tes) and (tes - dete <= te)) lcd.print(" (OK) ");
 }
else
{
if (dry == 1) 
...
if (h > hass) lcd.print(" (h>)");
if (h < hass - dehas) lcd.print(" (h<)");
if ((h <= hass) and (hass - dehas <= h)) lcd.print(" (OK) ");
}
...
lcd.clear();
for (long Counter = 0; Counter < 1500 ; Counter ++) { //returns to the main loop if it's been run 604 times 
